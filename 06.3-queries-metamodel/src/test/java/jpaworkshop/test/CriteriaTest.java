...
public class CriteriaTest {
    ...
    public void metamodelQuery() {
        ...
        cq.where(cb.equal(c.get(Employee_.id), 1));
        cq.select(c);
    ...
    public void employeePlz3() {
        ...
        cq.where(cb.like(emp.get(Employee_.address).get(Address_.zip), "3%"));
        cq.select(emp);
    ...
    public void avgSalary() {
        ...
        c.multiselect(emp.get(Employee_.department).get(Department_.name),
                cb.avg(emp.get(Employee_.salary))).groupBy(
                emp.get(Employee_.department).get(Department_.name));
    ...
    public void join() {
        ...
        CriteriaQuery<Employee> cq = cb.createQuery(Employee.class);
        Root<Employee> emp = cq.from(Employee.class);
        Join<Employee, Project> projects = emp.join(Employee_.projects);
        cq.where(cb.like(projects.get(Project_.name), "A%"));
        cq.select(emp);
